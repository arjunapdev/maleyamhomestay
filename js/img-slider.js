// JavaScript Document
// Get the modal
var modal1 = document.getElementById("myModal1");
var modal2 = document.getElementById("myModal2");
var modal3 = document.getElementById("myModal3");
var modal4 = document.getElementById("myModal4");
var modal5 = document.getElementById("myModal5");
var modal6 = document.getElementById("myModal6");
var modal7 = document.getElementById("myModal7");
var modal8 = document.getElementById("myModal8");
var modal9 = document.getElementById("myModal9");
var modal10 = document.getElementById("myModal10");
var modal11 = document.getElementById("myModal11");
var modal12 = document.getElementById("myModal12");
var modal13 = document.getElementById("myModal13");
var modal14 = document.getElementById("myModal14");
var modal15 = document.getElementById("myModal15");
var modal16 = document.getElementById("myModal16");
var modal17 = document.getElementById("myModal17");
var modal18 = document.getElementById("myModal18");
var modal19 = document.getElementById("myModal19");
var modal20 = document.getElementById("myModal20");
var modal21 = document.getElementById("myModal21");
var modal22 = document.getElementById("myModal22");
var modal23 = document.getElementById("myModal23");
var modal24 = document.getElementById("myModal24");
var modal25 = document.getElementById("myModal25");
// Get the image and insert it inside the modal - use its "alt" text as a caption

var img1= document.getElementById("myImg1");
var img2= document.getElementById("myImg2");
var img3= document.getElementById("myImg3");
var img4= document.getElementById("myImg4");
var img5= document.getElementById("myImg5");
var img6= document.getElementById("myImg6");
var img7= document.getElementById("myImg7");
var img8= document.getElementById("myImg8");
var img9= document.getElementById("myImg9");
var img10= document.getElementById("myImg10");
var img11= document.getElementById("myImg11");
var img12= document.getElementById("myImg12");
var img13= document.getElementById("myImg13");
var img14= document.getElementById("myImg14");
var img15= document.getElementById("myImg15");
var img16= document.getElementById("myImg16");
var img17= document.getElementById("myImg17");
var img18= document.getElementById("myImg18");
var img19= document.getElementById("myImg19");
var img20= document.getElementById("myImg20");
var img21= document.getElementById("myImg21");
var img22= document.getElementById("myImg22");
var img23= document.getElementById("myImg23");
var img24= document.getElementById("myImg24");
var img25= document.getElementById("myImg25");
var modalImg1 = document.getElementById("img1");
var modalImg2 = document.getElementById("img2");
var modalImg3 = document.getElementById("img3");
var modalImg4 = document.getElementById("img4");
var modalImg5 = document.getElementById("img5");
var modalImg6 = document.getElementById("img6");
var modalImg7 = document.getElementById("img7");
var modalImg8 = document.getElementById("img8");
var modalImg9 = document.getElementById("img9");
var modalImg10 = document.getElementById("img10");
var modalImg11 = document.getElementById("img11");
var modalImg12 = document.getElementById("img12");
var modalImg13 = document.getElementById("img13");
var modalImg14 = document.getElementById("img14");
var modalImg15 = document.getElementById("img15");
var modalImg16 = document.getElementById("img16");
var modalImg17 = document.getElementById("img17");
var modalImg18 = document.getElementById("img18");
var modalImg19 = document.getElementById("img19");
var modalImg20 = document.getElementById("img20");
var modalImg21 = document.getElementById("img21");
var modalImg22 = document.getElementById("img22");
var modalImg23 = document.getElementById("img23");
var modalImg24 = document.getElementById("img24");
var modalImg25 = document.getElementById("img25");
img1.onclick = function(){
    modal1.style.display = "block";
    modalImg1.src = "img/Gallery/big1.jpg";    
}
img2.onclick = function(){
    modal2.style.display = "block";
    modalImg2.src = "img/Gallery/big2.jpg";
    
}
img3.onclick = function(){
    modal3.style.display = "block";
    modalImg3.src = "img/Gallery/big3.jpg";
  /*   */
}
img4.onclick = function(){
    modal4.style.display = "block";
    modalImg4.src = "img/Gallery/big4.jpg";
    
}
img5.onclick = function(){
    modal5.style.display = "block";
    modalImg5.src = "img/Gallery/big5.jpg";
    
}
img6.onclick = function(){
    modal6.style.display = "block";
    modalImg6.src = "img/Gallery/big6.jpg";
    
}
img7.onclick = function(){
    modal7.style.display = "block";
    modalImg7.src = "img/Gallery/big7.jpg";
    
}
img8.onclick = function(){
    modal8.style.display = "block";
    modalImg8.src = "img/Gallery/big8.jpg";
    
}
img9.onclick = function(){
    modal9.style.display = "block";
    modalImg9.src = "img/Gallery/big9.jpg";
    
}
img10.onclick = function(){
    modal10.style.display = "block";
    modalImg10.src = "img/Gallery/big10.jpg";
    
}
img11.onclick = function(){
    modal11.style.display = "block";
    modalImg11.src = "img/Gallery/big11.jpg";
    
}
img12.onclick = function(){
    modal12.style.display = "block";
    modalImg12.src = "img/Gallery/big12.jpg";
    
}
img13.onclick = function(){
    modal13.style.display = "block";
    modalImg13.src = "img/Gallery/big13.jpg";
    
}
img14.onclick = function(){
    modal14.style.display = "block";
    modalImg14.src = "img/Gallery/big14.jpg";
    
}
img15.onclick = function(){
    modal15.style.display = "block";
    modalImg15.src = "img/Gallery/big15.jpg";
    
}
img16.onclick = function(){
    modal16.style.display = "block";
    modalImg16.src = "img/Gallery/big16.jpg";
    
}
img17.onclick = function(){
    modal17.style.display = "block";
    modalImg17.src = "img/Gallery/big17.jpg";
    
}
img18.onclick = function(){
    modal18.style.display = "block";
    modalImg18.src = "img/Gallery/big18.jpg";
    
}
img19.onclick = function(){
    modal19.style.display = "block";
    modalImg19.src = "img/Gallery/big19.jpg";
    
}
img20.onclick = function(){
    modal20.style.display = "block";
    modalImg20.src = "img/Gallery/big20.jpg";
    
}
img21.onclick = function(){
    modal21.style.display = "block";
    modalImg21.src = "img/Gallery/big21.jpg";
}
img22.onclick = function(){
    modal22.style.display = "block";
    modalImg22.src = "img/Gallery/big22.jpg";
}
img23.onclick = function(){
    modal23.style.display = "block";
    modalImg23.src = "img/Gallery/big23.jpg";
}
img24.onclick = function(){
    modal24.style.display = "block";
    modalImg24.src = "img/Gallery/big24.jpg";
}
img25.onclick = function(){
    modal25.style.display = "block";
    modalImg25.src = "img/Gallery/big25.jpg";
}
// Get the <cls1> element that closes the modal
var cls1 = document.getElementsByClassName("close1")[0];
var cls2 = document.getElementsByClassName("close2")[0];
var cls3 = document.getElementsByClassName("close3")[0];
var cls4 = document.getElementsByClassName("close4")[0];
var cls5 = document.getElementsByClassName("close5")[0];
var cls6 = document.getElementsByClassName("close6")[0];
var cls7 = document.getElementsByClassName("close7")[0];
var cls8 = document.getElementsByClassName("close8")[0];
var cls9 = document.getElementsByClassName("close9")[0];
var cls10 = document.getElementsByClassName("close10")[0];
var cls11 = document.getElementsByClassName("close11")[0];
var cls12 = document.getElementsByClassName("close12")[0];
var cls13 = document.getElementsByClassName("close13")[0];
var cls14 = document.getElementsByClassName("close14")[0];
var cls15 = document.getElementsByClassName("close15")[0];
var cls16 = document.getElementsByClassName("close16")[0];
var cls17 = document.getElementsByClassName("close17")[0];
var cls18 = document.getElementsByClassName("close18")[0];
var cls19 = document.getElementsByClassName("close19")[0];
var cls20 = document.getElementsByClassName("close20")[0];
var cls21 = document.getElementsByClassName("close21")[0];
var cls22 = document.getElementsByClassName("close22")[0];
var cls23 = document.getElementsByClassName("close23")[0];
var cls24 = document.getElementsByClassName("close24")[0];
var cls25 = document.getElementsByClassName("close25")[0];

// When the user clicks on <cls1> (x), close the modal
cls1.onclick = function() { 
    modal1.style.display = "none";
}
cls2.onclick = function() { 
    modal2.style.display = "none";
}
cls3.onclick = function() { 
    modal3.style.display = "none";
}
cls4.onclick = function() { 
    modal4.style.display = "none";
}
cls5.onclick = function() { 
    modal5.style.display = "none";
}
cls6.onclick = function() { 
    modal6.style.display = "none";
}
cls7.onclick = function() { 
    modal7.style.display = "none";
}
cls8.onclick = function() { 
    modal8.style.display = "none";
}
cls9.onclick = function() { 
    modal9.style.display = "none";
}
cls10.onclick = function() { 
    modal10.style.display = "none";
}
cls11.onclick = function() { 
    modal11.style.display = "none";
}
cls12.onclick = function() { 
    modal12.style.display = "none";
}
cls13.onclick = function() { 
    modal13.style.display = "none";
}
cls14.onclick = function() { 
    modal14.style.display = "none";
}
cls15.onclick = function() { 
    modal15.style.display = "none";
}
cls16.onclick = function() { 
    modal16.style.display = "none";
}
cls17.onclick = function() { 
    modal17.style.display = "none";
}
cls18.onclick = function() { 
    modal18.style.display = "none";
}
cls19.onclick = function() { 
    modal19.style.display = "none";
}
cls20.onclick = function() { 
    modal20.style.display = "none";
}
cls21.onclick = function() { 
    modal21.style.display = "none";
}
cls22.onclick = function() { 
    modal22.style.display = "none";
}
cls23.onclick = function() { 
    modal23.style.display = "none";
}
cls24.onclick = function() { 
    modal24.style.display = "none";
}
cls25.onclick = function() { 
    modal25.style.display = "none";
}
var previous1 = document.getElementById("lt1");
var previous2 = document.getElementById("lt2");
var previous3 = document.getElementById("lt3");
var previous4 = document.getElementById("lt4");
var previous5 = document.getElementById("lt5");
var previous6 = document.getElementById("lt6");
var previous7 = document.getElementById("lt7");
var previous8 = document.getElementById("lt8");
var previous9 = document.getElementById("lt9");
var previous10 = document.getElementById("lt10");
var previous11 = document.getElementById("lt11");
var previous12 = document.getElementById("lt12");
var previous13 = document.getElementById("lt13");
var previous14 = document.getElementById("lt14");
var previous15 = document.getElementById("lt15");
var previous16 = document.getElementById("lt16");
var previous17 = document.getElementById("lt17");
var previous18 = document.getElementById("lt18");
var previous19 = document.getElementById("lt19");
var previous20 = document.getElementById("lt20");
var previous21 = document.getElementById("lt21");
var previous22 = document.getElementById("lt22");
var previous23 = document.getElementById("lt23");
var previous24 = document.getElementById("lt24");
var previous25 = document.getElementById("lt25");



previous1.onclick = function(){
	modal1.style.display = "none";
	modal25.style.display = "block";
    modalImg25.src = "img/Gallery/big25.jpg";
}
previous2.onclick = function(){
	modal2.style.display = "none";
	modal1.style.display = "block";
    modalImg1.src = "img/Gallery/big1.jpg";
}
previous3.onclick = function(){
	modal3.style.display = "none";
	modal2.style.display = "block";
    modalImg2.src = "img/Gallery/big2.jpg";
}
previous4.onclick = function(){
	modal4.style.display = "none";
	modal3.style.display = "block";
    modalImg3.src = "img/Gallery/big3.jpg";
}
previous5.onclick = function(){
	modal5.style.display = "none";
	modal4.style.display = "block";
    modalImg4.src = "img/Gallery/big4.jpg";
}
previous6.onclick = function(){
	modal6.style.display = "none";
	modal5.style.display = "block";
    modalImg5.src = "img/Gallery/big5.jpg";
}
previous7.onclick = function(){
	modal7.style.display = "none";
	modal6.style.display = "block";
    modalImg6.src = "img/Gallery/big6.jpg";
}
previous8.onclick = function(){
	modal8.style.display = "none";
	modal7.style.display = "block";
    modalImg7.src = "img/Gallery/big7.jpg";
}
previous9.onclick = function(){
	modal9.style.display = "none";
	modal8.style.display = "block";
    modalImg8.src = "img/Gallery/big8.jpg";
}
previous10.onclick = function(){
	modal10.style.display = "none";
	modal9.style.display = "block";
    modalImg9.src = "img/Gallery/big9.jpg";
}
previous11.onclick = function(){
	modal11.style.display = "none";
	modal10.style.display = "block";
    modalImg10.src = "img/Gallery/big10.jpg";
}
previous12.onclick = function(){
	modal12.style.display = "none";
	modal11.style.display = "block";
    modalImg11.src = "img/Gallery/big11.jpg";
}
previous13.onclick = function(){
	modal13.style.display = "none";
	modal12.style.display = "block";
    modalImg12.src = "img/Gallery/big12.jpg";
}
previous14.onclick = function(){
	modal14.style.display = "none";
	modal13.style.display = "block";
    modalImg13.src = "img/Gallery/big13.jpg";
}
previous15.onclick = function(){
	modal15.style.display = "none";
	modal14.style.display = "block";
    modalImg14.src = "img/Gallery/big14.jpg";
}
previous16.onclick = function(){
	modal16.style.display = "none";
	modal15.style.display = "block";
    modalImg15.src = "img/Gallery/big15.jpg";
}
previous17.onclick = function(){
	modal17.style.display = "none";
	modal16.style.display = "block";
    modalImg16.src = "img/Gallery/big16.jpg";
}
previous18.onclick = function(){
	modal18.style.display = "none";
	modal17.style.display = "block";
    modalImg17.src = "img/Gallery/big17.jpg";
}
previous19.onclick = function(){
	modal19.style.display = "none";
	modal18.style.display = "block";
    modalImg18.src = "img/Gallery/big18.jpg";
}
previous20.onclick = function(){
	modal20.style.display = "none";
	modal19.style.display = "block";
    modalImg19.src = "img/Gallery/big19.jpg";
}
previous21.onclick = function(){
	modal21.style.display = "none";
	modal20.style.display = "block";
    modalImg20.src = "img/Gallery/big20.jpg";
}
previous22.onclick = function(){
	modal22.style.display = "none";
	modal21.style.display = "block";
    modalImg21.src = "img/Gallery/big21.jpg";
}
previous23.onclick = function(){
    modal23.style.display = "none";
    modal22.style.display = "block";
    modalImg22.src = "img/Gallery/big22.jpg";
}
previous24.onclick = function(){
    modal24.style.display = "none";
    modal23.style.display = "block";
    modalImg23.src = "img/Gallery/big23.jpg";
}
previous25.onclick = function(){
    modal25.style.display = "none";
    modal24.style.display = "block";
    modalImg24.src = "img/Gallery/big24.jpg";
}
var next1 = document.getElementById("gt1");
var next2 = document.getElementById("gt2");
var next3 = document.getElementById("gt3");
var next4 = document.getElementById("gt4");
var next5 = document.getElementById("gt5");
var next6 = document.getElementById("gt6");
var next7 = document.getElementById("gt7");
var next8 = document.getElementById("gt8");
var next9 = document.getElementById("gt9");
var next10 = document.getElementById("gt10");
var next11 = document.getElementById("gt11");
var next12 = document.getElementById("gt12");
var next13 = document.getElementById("gt13");
var next14 = document.getElementById("gt14");
var next15 = document.getElementById("gt15");
var next16 = document.getElementById("gt16");
var next17 = document.getElementById("gt17");
var next18 = document.getElementById("gt18");
var next19 = document.getElementById("gt19");
var next20 = document.getElementById("gt20");
var next21 = document.getElementById("gt21");
var next22 = document.getElementById("gt22");
var next23 = document.getElementById("gt23");
var next24 = document.getElementById("gt24");
var next25 = document.getElementById("gt25");



next1.onclick = function(){
	modal1.style.display = "none";
	modal2.style.display = "block";
    modalImg2.src = "img/Gallery/big2.jpg";
}
next2.onclick = function(){
	modal2.style.display = "none";
	modal3.style.display = "block";
    modalImg3.src = "img/Gallery/big3.jpg";
}
next3.onclick = function(){
	modal3.style.display = "none";
	modal4.style.display = "block";
    modalImg4.src = "img/Gallery/big4.jpg";
}
next4.onclick = function(){
	modal4.style.display = "none";
	modal5.style.display = "block";
    modalImg5.src = "img/Gallery/big5.jpg";
}
next5.onclick = function(){
	modal5.style.display = "none";
	modal6.style.display = "block";
    modalImg6.src = "img/Gallery/big6.jpg";
}
next6.onclick = function(){
	modal6.style.display = "none";
	modal7.style.display = "block";
    modalImg7.src = "img/Gallery/big7.jpg";
}
next7.onclick = function(){
	modal7.style.display = "none";
	modal8.style.display = "block";
    modalImg8.src = "img/Gallery/big8.jpg";
}
next8.onclick = function(){
	modal8.style.display = "none";
	modal9.style.display = "block";
    modalImg9.src = "img/Gallery/big9.jpg";
}
next9.onclick = function(){
	modal9.style.display = "none";
	modal10.style.display = "block";
    modalImg10.src = "img/Gallery/big10.jpg";
}
next10.onclick = function(){
	modal10.style.display = "none";
	modal11.style.display = "block";
    modalImg11.src = "img/Gallery/big11.jpg";
}
next11.onclick = function(){
	modal11.style.display = "none";
	modal12.style.display = "block";
    modalImg12.src = "img/Gallery/big12.jpg";
}
next12.onclick = function(){
	modal12.style.display = "none";
	modal13.style.display = "block";
    modalImg13.src = "img/Gallery/big13.jpg";
}
next13.onclick = function(){
	modal13.style.display = "none";
	modal14.style.display = "block";
    modalImg14.src = "img/Gallery/big14.jpg";
}
next14.onclick = function(){
	modal14.style.display = "none";
	modal15.style.display = "block";
    modalImg15.src = "img/Gallery/big15.jpg";
}
next15.onclick = function(){
	modal15.style.display = "none";
	modal16.style.display = "block";
    modalImg16.src = "img/Gallery/big16.jpg";
}
next16.onclick = function(){
	modal16.style.display = "none";
	modal17.style.display = "block";
    modalImg17.src = "img/Gallery/big17.jpg";
}
next17.onclick = function(){
	modal17.style.display = "none";
	modal18.style.display = "block";
    modalImg18.src = "img/Gallery/big18.jpg";
}
next18.onclick = function(){
	modal18.style.display = "none";
	modal19.style.display = "block";
    modalImg19.src = "img/Gallery/big19.jpg";
}
next19.onclick = function(){
	modal19.style.display = "none";
	modal20.style.display = "block";
    modalImg20.src = "img/Gallery/big20.jpg";
}
next20.onclick = function(){
	modal20.style.display = "none";
	modal21.style.display = "block";
    modalImg21.src = "img/Gallery/big21.jpg";
}
next21.onclick = function(){
	modal21.style.display = "none";
	modal22.style.display = "block";
    modalImg22.src = "img/Gallery/big22.jpg";
}
next22.onclick = function(){
	modal22.style.display = "none";
	modal23.style.display = "block";
    modalImg23.src = "img/Gallery/big23.jpg";
}
next23.onclick = function(){
    modal23.style.display = "none";
    modal24.style.display = "block";
    modalImg24.src = "img/Gallery/big24.jpg";
}
next24.onclick = function(){
    modal24.style.display = "none";
    modal5.style.display = "block";
    modalImg25.src = "img/Gallery/big25.jpg";
}
next25.onclick = function(){
    modal25.style.display = "none";
    modal1.style.display = "block";
    modalImg1.src = "img/Gallery/big1.jpg";
}